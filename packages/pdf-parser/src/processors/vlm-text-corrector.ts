 ...
 */
const TEXT_CORRECTION_SYSTEM_PROMPT = `You are a text correction engine for OCR output. Compare OCR text against the page image and reference text to fix errors.
...
FIX: garbled/wrong Chinese characters, mojibake, encoding artifacts, random ASCII/Korean replacing Hanja
...
- The ref text is the AUTHORITATIVE source for Chinese characters (漢字/Hanja)
...
export class VlmTextCorrector {
  ...
  private async correctPage(
    ...
    try {
      ...
      const references = pageText
        ? this.matchTextToReference(pageTexts, pageText)
        : undefined;
  ...
  matchTextToReference(
    ...
    for (let promptIndex = 0; promptIndex < pageTexts.length; promptIndex++) {
      ...
      if (
        bestBlockIndex >= 0 &&
        bestScore >= REFERENCE_MATCH_THRESHOLD &&
        refBlocks[bestBlockIndex] !== ocrText
      ) {
        result.set(promptIndex, refBlocks[bestBlockIndex]);
