...
> & {
  ...
  /** Token usage aggregator for tracking post-Docling correction calls */
  aggregator?: LLMTokenUsageAggregator;
  /** Callback fired after correction work completes, with cumulative token usage */
...
export class PDFConverter {
  ...
  async convert(
    ...
    let conversionReport: TokenUsageReport | null;
    if (trackedOptions.chunkedConversion && url.startsWith('file://')) {
      const chunked = new ChunkedPDFConverter(
        this.logger,
        this.client,
        {
          chunkSize:
            trackedOptions.chunkSize ?? CHUNKED_CONVERSION.DEFAULT_CHUNK_SIZE,
          maxRetries:
            trackedOptions.chunkMaxRetries ??
            CHUNKED_CONVERSION.DEFAULT_MAX_RETRIES,
        },
        this.timeout,
      );
      try {
        conversionReport = await chunked.convertChunked(
          url,
          reportId,
          correctedOnComplete,
        ...
        conversionReport = await this.convertViaImagePdf(
          url,
          reportId,
          correctedOnComplete,
          cleanupAfterCallback,
    ...
    } else if (trackedOptions.forceImagePdf) {
      conversionReport = await this.convertViaImagePdf(
        url,
        reportId,
        correctedOnComplete,
      ...
      conversionReport = await this.convertWithFallback(
        url,
        reportId,
        correctedOnComplete,
        cleanupAfterCallback,
   ...
   * Returns null when no LLM calls were tracked.
   */
