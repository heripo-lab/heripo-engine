...
Table cells intentionally do not have cell-level `sourceRef` values. Use
`table.sourceRef` together with `grid[row][col]` when tracing a cell back to
the source table.
...
Types for optional page-level review assistance generated by
`@heripo/pdf-parser` when `reviewAssistance` is enabled.
...
// Bcp47LanguageTag - Union type of supported full BCP-47 language tags
type Bcp47LanguageTag = (typeof BCP47_LANGUAGE_TAGS)[number];

// BCP47_LANGUAGE_TAGS - Const array of supported ocrmac language tags
