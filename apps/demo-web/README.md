...
- Ledger extraction preview from a `processed-document.json` upload (skips PDF parsing entirely)
...
### 5. Ledger Extraction (Preview)

The `Ledger Extraction` stage in the pipeline stepper runs `@heripo/ledger-extractor` directly on a preprocessed document, skipping PDF parsing and document preprocessing:

1. Select **Ledger Extraction** in the pipeline stepper on the home page
2. Attach a single `processed-document.json` file (max 100MB; ZIP archives are not accepted)
3. Click "Start Ledger Extraction" — the file is validated (extension/MIME → size → JSON parse → `ProcessedDocument` runtime validation) and a ledger task is queued
4. The **preprocessed document validation page** (`/ledger/validation/[taskId]`) shows the `LedgerExtractionPreview`: reportId, schemaVersion, counts, sample chapter titles / text blocks / image URLs / tables, and the raw preview JSON

Notes:

- This stage is **preview-only**: it summarizes the input to prove it arrived intact. Actual ledger extraction (LLM-based, domain schema, DB storage) is follow-up work.
- The validation page is a temporary way to confirm the upload; the actual ledger result page (`/result/[taskId]`) is a titled placeholder until its design is decided.
- The JSON must conform to the `ProcessedDocument` type in `@heripo/model` (`packages/model/src/types/processed-document.ts`).
- `ProcessedImage.path` values must be full public CDN URLs (as generated by the `heripo-web` export script). They are consumed as-is and never fetched or re-signed by the demo.
- Test JSON files are produced by the `heripo-web` export script, or by exporting the processed result JSON from a completed Raw Data Extraction task.
...
- `@heripo/ledger-extractor`: Ledger extraction preview
