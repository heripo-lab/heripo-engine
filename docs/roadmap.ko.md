...
| 항목              | 설명                                                                                          | 상태         |
| ----------------- | --------------------------------------------------------------------------------------------- | ------------ |
| **혼합 문자 OCR** | 한글·한자 혼용 문서를 자동 감지하여 VLM으로 보정                                              | ✅ 해결됨    |
...
- 고품질 한국어 문서 인식
- 혼합 문자 자동 감지 및 VLM 보정 — ocrmac은 대량 처리에 탁월하지만(무료, GPU 가속, 고품질) 혼합 문자 체계(한글·한자 등)를 처리하지 못하므로, 해당 페이지만 자동 감지하여 VLM으로 보정
- 이미지 자동 추출
