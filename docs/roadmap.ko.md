...
| 항목                  | 설명                                                                                          | 상태         |
| --------------------- | --------------------------------------------------------------------------------------------- | ------------ |
| **한국어 보고서 OCR** | 한국어 보고서를 자동 감지하여 VLM으로 보정                                                    | ✅ 해결됨    |
| **목차 의존성**       | 목차 없는 보고서는 처리 실패 (의도된 동작). 드문 추출 실패는 human intervention으로 대응 예정 | 📋 설계 의도 |
| **세로쓰기 문서**     | 페이지 번호가 한자인 오래된 세로쓰기 문서 (장기 목표, 현재 계획 없음)                         | 📅 장기 목표 |

**PDF 파싱 및 OCR**

- Docling SDK를 활용한 PDF 파싱
- Apple Silicon GPU 가속 OCR (ocrmac / Apple Vision Framework)
- 고품질 한국어 문서 인식
- 한국어 보고서 감지 및 VLM 보정 — ocrmac은 대량 처리에 탁월하지만(무료, GPU 가속, 고품질), 한국어 고고학 보고서는 한자 복원과 문자 체계 인식 보정을 위해 전체 페이지에 VLM 텍스트 보정을 적용
- 이미지 자동 추출
