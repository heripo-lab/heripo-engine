...
| **VLM**    | GPT, Gemini 등 (Vision)      | 전처리 1         | 혼합 문자 감지 및 페이지별 텍스트 보정, API 호출 |
...
| **혼합 문자 감지**   | 텍스트 레이어 사전 검사 + VLM 샘플링으로 한글·한자 혼용 자동 감지 | VLM (Vision LLM)      |
| **VLM 텍스트 보정**  | 혼합 문자 페이지별 텍스트 보정                                    | VLM (Vision LLM)      |
...
OCR 전략 시스템은 핵심 과제를 해결합니다: **ocrmac(Apple Vision Framework)은 대량 처리에 매우 우수합니다** — 무료이고, GPU 가속을 지원하며, 고품질 결과를 제공하여 수천~수백만 권의 보고서 처리에 최적의 솔루션입니다. **그러나 ocrmac은 혼합 문자 체계를 처리하지 못합니다** (예: 한글·한자 혼용).

이를 해결하기 위해 **2단계 감지 + 타겟 보정** 방식을 사용합니다:

1. **1단계: 텍스트 레이어 사전 검사** (비용 없음) — `pdftotext`로 문서의 텍스트 레이어에서 한글과 CJK 문자를 확인합니다. 둘 다 존재하면 혼합 문자 문서로 판별합니다.
2. **2단계: VLM 샘플링** (필요 시에만) — 최대 15페이지를 샘플링하여 Vision LLM으로 혼합 문자 존재를 확인합니다. 최초 감지 시 즉시 종료하여 API 비용을 최소화합니다.
3. **타겟 보정** — 혼합 문자 문제가 있는 페이지만 VLM에 전송하여 보정합니다. 나머지 문서는 빠른 ocrmac 파이프라인을 그대로 사용하여 비용과 시간을 최소화합니다.

이 접근 방식은 대부분의 문서를 빠르고 무료인 ocrmac 엔진으로 처리하면서, 소수의 혼합 문자 페이지만 VLM으로 타겟 보정합니다.
