...
| **VLM**    | GPT, Gemini 등 (Vision)      | 전처리 1         | 한국어 보고서 감지 및 전체 문서 텍스트 보정, API 호출 |
...
flowchart LR
    subgraph PDFParser[PDF Parser - macOS]
        direction TB
        A[PDF 파일 수신] --> B[OCR 전략 샘플링]
        B -->|ocrmac| C[OCR 처리 - Apple Vision]
        B -->|vlm| C
        C --> D[구조 분석 - Docling]
        D --> E[이미지 추출]
        E --> F{한국어 보고서 감지?}
...
| **한국어 보고서 감지** | 텍스트 레이어 사전 검사 + VLM 언어 샘플링으로 한국어 문서 자동 감지 | VLM (Vision LLM)      |
| **VLM 텍스트 보정**    | 한국어 보고서 전체 페이지 OCR 텍스트 보정                           | VLM (Vision LLM)      |
| **문서 유형 검증**     | 처리 전 고고학 보고서인지 검증                                      | LLM                   |
| **구조 분석**          | 텍스트, 표, 이미지 영역 자동 식별                                   | Docling (규칙 + ML)   |
| **청크 변환**          | 대용량 PDF를 청크 단위로 분할하여 안정적 처리                       | -                     |
| **이미지 PDF 폴백**    | 직접 변환 실패 시 PDF를 이미지로 변환 후 처리                       | -                     |
| **이미지 추출**        | 문서 내 모든 이미지 파일로 저장                                     | -                     |
| **페이지 이미지**      | 각 페이지를 이미지로 저장 (LLM Vision용)                            | -                     |
| **서버 크래시 복구**   | 연결 실패 시 docling-serve 자동 재시작                              | -                     |

#### OCR 전략 시스템

OCR 전략 시스템은 핵심 과제를 해결합니다: **ocrmac(Apple Vision Framework)은 대량 처리에 매우 우수합니다** — 무료이고, GPU 가속을 지원하며, 고품질 결과를 제공하여 수천~수백만 권의 보고서 처리에 최적의 솔루션입니다. **그러나 한국어 고고학 보고서는 표준 OCR만으로 안정적으로 처리하기 어려운 한자 복원과 문자 체계 인식 보정이 자주 필요합니다.**

이를 해결하기 위해 **2단계 한국어 문서 감지 + 전체 문서 보정** 방식을 사용합니다:

1. **1단계: 텍스트 레이어 사전 검사** (비용 없음) — `pdftotext`로 문서의 텍스트 레이어에서 한글을 확인합니다. 한글이 있으면 한국어 문서로 판별합니다.
2. **2단계: VLM 샘플링** (필요 시에만) — 최대 15페이지를 샘플링하여 Vision LLM으로 언어를 판정합니다. `ko-KR` 감지 시 즉시 종료합니다.
3. **전체 문서 보정** — 한국어 보고서는 표준 OCR 파이프라인을 먼저 거친 뒤 모든 페이지를 VLM 텍스트 보정에 전달합니다.

이 접근 방식은 비한국어 문서를 빠르고 무료인 ocrmac 엔진으로 처리하면서, 한국어 고고학 보고서에는 문서 전체에 일관된 VLM 보정을 적용합니다.
...
ProcessedDocument
├── reportId           # 보고서 고유 ID
├── pageRangeMap       # PDF 페이지 → 실제 페이지 매핑 (Record<number, PageRange>)
├── chapters[]         # 목차 기반 챕터 구조
│   ├── originTitle    # 보고서 원본 제목
│   ├── title          # 정리된 챕터 제목
│   ├── textBlocks[]   # 챕터 내 텍스트들
│   ├── imageIds[]     # 연결된 이미지 ID들
│   ├── tableIds[]     # 연결된 표 ID들
│   ├── footnoteIds[]  # 연결된 각주 ID들
│   └── children[]     # 하위 챕터들
├── images[]           # 이미지 정보 (캡션, 위치 등)
├── tables[]           # 표 정보 (캡션, 셀 데이터 등)
└── footnotes[]        # 각주 정보 (텍스트, 페이지 번호)
```
