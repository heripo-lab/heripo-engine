...
- [소개](#소개)
  ...
  - [데이터 파이프라인](#데이터-파이프라인)
- [엔진 활용 사례: heripo 베이스캠프](#엔진-활용-사례-heripo-베이스캠프)
- [주요 기능](#주요-기능)
- [아키텍처](#아키텍처)
- [설치](#설치)
- [패키지](#패키지)
...
- Research:
  - 「대형 언어 모델(LLM)을 활용한 고고학 정보화 연구 -발굴조사보고서의 메타데이터 자동 추출 파이프라인 개념 검증-」 (2025, 『헤리티지: 역사와 과학』 제58권 제3호, KCI 등재)
    ...
    - [한국어 원문](https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART003244876)
...
#### 김가은 (Software Engineer)

- Role: 고고학 연구 플랫폼 개발
- Background: 소프트웨어 엔지니어 (고고학 석사 수료, 문헌정보학 학사)
...
## 엔진 활용 사례: heripo 베이스캠프

**heripo 베이스캠프**는 비공개 알파 테스트 중인 연구 지원 도구입니다. 엔진 데모와는 별도로 운영되는 서비스로, heripo engine을 자체 발굴조사보고서 데이터 구축에 활용합니다.

**한국에서 시작해, 국가 경계를 넘어.** 초기에는 한국의 발굴조사보고서를 대상으로 시작하지만, 서비스의 범위를 한국에 한정하지 않습니다. 여러 국가와 지역의 보고서를 함께 탐색하며 연구 자료를 찾을 수 있는 서비스로 확장해 나가는 것을 목표로 합니다.

- **지도 기반 보고서 탐색**: 지도 위에서 보고서를 찾고, 조건과 AI 필터로 탐색 범위를 좁힙니다.
- **보고서 내용 탐색**: 원문 PDF와 텍스트·이미지·표를 함께 살펴보는 리더를 제공합니다.
- **구조화 데이터의 서비스 활용**: 엔진이 PDF 추출과 문서 구조화를 담당하고, 베이스캠프가 연구자를 위한 탐색 기능과 인터페이스를 더합니다.

[엔진 데모](https://engine-demo.heripo.org)는 PDF 처리 기능을 체험하는 공간이고, [heripo 베이스캠프](https://heripo.app)는 같은 엔진을 활용해 자체 데이터로 운영하는 별도의 서비스입니다. 데모에 업로드한 보고서와 처리 결과는 베이스캠프에 반영되지 않습니다.
...
- **원천 추적성**: 선택적으로 Docling 원천 metadata와 node-level source reference 보존
- **테이블 그리드 정규화**: row/column span을 보존하고 병합 셀의 shadow entry 제거
...
- **ReviewAssistanceReport**: 선택적 page-level review assistance 리포트 모델
...
- [@heripo/logger](./packages/logger/README.ko.md)
