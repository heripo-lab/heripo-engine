...
> ⚠️ **알려진 제한사항 (v0.1.x)**:
>
> - **한자 OCR**: 한글/한자 혼용 문서에서 한자가 깨질 수 있음 (개선 진행중)
> - **목차 추출**: 목차가 없거나 특이한 구조면 추출 실패 가능. 특이사항이 없어 보이는 목차도 일부 실패하는 케이스 있음 (지속 개선중)
> - **세로쓰기 문서**: 페이지 번호가 한자인 오래된 세로쓰기 문서는 장기적으로 지원 대상이지만, 현재는 계획에 없음
> - **자동화 범위**: 완전 자동화를 지향하지 않음. 100% 수동이던 작업을 90% 자동화하는 것이 목표. 페이지 맵핑/목차 추출은 파이프라인의 근간이나 100%에 가까운 자동화 불가능—보편적 패턴에 집중하며 최적의 지점에서 멈춤. 직접 보기에도 어려운 보고서는 실패가 정상. 이후 단계(유구/유물 추출)는 100%에 가까운 자동화 지향. 특이 케이스는 하이브리드 방식으로 대응 예정
...
- Research:
  - [「대형 언어 모델(LLM)을 활용한 고고학 정보화 연구 -발굴조사보고서의 메타데이터 자동 추출 파이프라인 개념 검증-」](https://koreascience.kr/article/JAKO202570361249829.page) (2025, KCI 등재)
